...
### Client Installation
Strelka's core client applications are written in Go and can be run natively on a host or inside of a container.
...
## Components
### Client-Side
Strelka's core client applications are designed to efficiently integrate a wide-range of systems (Windows, Mac, Linux) with a cluster. Out of the box client applications are written in Go and custom clients can be written in any language supported by gRPC.
...
#### strelka-filestream
This client app is designed to continuously stream files and retrieves their results. This app is intended for use on systems that continuously generate files, such as network security monitoring (NSM) sensors, email gateways, and web proxies.
