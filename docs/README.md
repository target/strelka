...
| ScanEmail         | Collects metadata (curated headers plus parsed SPF/DKIM/DMARC/compauth and Exchange spam signals, plus body links) and extracts files from email messages Body is populated from the `text/plain` part when present; falls back to HTML-stripped text when the message is HTML-only.                                                                                                                           | `body_enabled` -- Enable body extraction (defaults to `True`)<br>`body_chars` -- Maximum characters stored in the `body` field, split evenly between head and tail with `...`; `-1` for unlimited (defaults to `200`)<br>`max_links` -- Max number of de-duplicated body links (URLs) to emit (defaults to `50`)<br>`capture_raw_headers` -- Opt-in (default `False`): emit the full raw header map under `headers`. Emits a dynamic-keyed object, so enable only with a `flattened`/`flat_object` index mapping<br>`max_header_length` -- Per-value truncation cap in bytes for the raw header map (defaults to `2048`)<br>`max_headers_total` -- Total size cap in bytes for the raw header map, `0` for unlimited (defaults to `32768`)<br>`header_skip_list` -- Header names excluded from the raw header map (large/low-value headers)<br>`link_rewrite_mode` -- Controls gateway-link unwrapping: `"none"` (default) disables it; `"replace"` replaces wrapped URLs in `links` with destinations; `"copy"` appends unwrapped destinations to `links` and emits them in `links_unwrapped`; `"parallel"` leaves `links` unchanged and emits only the unwrapped destinations in `links_unwrapped`<br>`link_rewrite_rules` -- List of gateway unwrap rules; each rule defines a URL pattern and how to extract the destination (required for `link_rewrite_mode` to take effect)<br>`link_rewrite_urldecode` -- Whether to URL-decode extracted gateway destinations: `"auto"` (default) decodes only when the value appears percent-encoded; `"always"` or `"never"` force the behaviour<br>`mailbox_mode` -- Controls display-name surfacing in address fields: `"none"` (default) emits addr-specs only; `"copy"` appends `"Display Name <addr>"` strings to the same field; `"parallel"` keeps addr-specs in the primary field and emits formatted addresses in a sibling `*_mailbox` field                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
