...
| ScanPcap          | Extracts files from PCAP/PCAPNG using Zeek, matches optionally provided Suricata signatures                                                                                       | `limit` -- maximum number of files to extract (defaults to `1000`)<br>`suricata_config` -- path to the Suricata configuration file (defaults to `/etc/strelka/suricata/suricata.yaml`)<br>`suricata_rules` -- path to the Suricata rules file (defaults to `/etc/strelka/suricata/suricata.rules`)<br>`suricata_alert_dedupe` -- deduplicate Suricata alerts (defaults to `False`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | [Ryan O'Horo](https://github.com/ryanohoro)                                                     |
