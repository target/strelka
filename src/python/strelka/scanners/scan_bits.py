...
from bits.structs import CONTROL, FILE, JOB
...
WIN10_FILE_DELIMITER = (
    b"\xE4\xCF\x9E\x51\x46\xD9\x97\x43\xB7\x3E\x26\x85\x13\x05\x1A\xB2"
)
...
class ScanBITS(strelka.Scanner):
    ...
    def parse_qmgr10_job(self, file_entries, job_data):
        ...
        try:
            ...
            print(
                f"Exception occurred parsing job: {traceback.format_exc()}",
                file=sys.stderr,
            )
            return None
    ...
    def parse_qmgr10_file(self, file_data, suppress_duplicates):
        ...
        try:
            ...
            if filetime != 0:
                cur_job["ctime"] = datetime.datetime(1601, 1, 1) + datetime.timedelta(
