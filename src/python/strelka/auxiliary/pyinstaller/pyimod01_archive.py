...
class ZlibArchiveReader:
    ...
    def __init__(self, data, start_offset=None, check_pymagic=False):
        self._data = data
        ...
        with io.BytesIO(self._data) as fd:
            ...
            fd.seek(self._start_offset, os.SEEK_SET)

            magic = fd.read(len(self._PYZ_MAGIC_PATTERN))
            if magic != self._PYZ_MAGIC_PATTERN:
                raise ArchiveReadError("PYZ magic pattern mismatch!")

            # Read python magic/version number
            pymagic = fd.read(len(PYTHON_MAGIC_NUMBER))
            if check_pymagic and pymagic != PYTHON_MAGIC_NUMBER:
                raise ArchiveReadError("Python magic pattern mismatch!")

            # Read TOC offset
            toc_offset, *_ = struct.unpack("!i", fd.read(4))

            # Load TOC
            fd.seek(self._start_offset + toc_offset, os.SEEK_SET)
            self.toc = dict(marshal.load(fd))
    ...
    def extract(self, name, raw=False):
        ...
        try:
            with io.BytesIO(self._data) as fd:
                fd.seek(self._start_offset + entry_offset)
                obj = fd.read(entry_length)
